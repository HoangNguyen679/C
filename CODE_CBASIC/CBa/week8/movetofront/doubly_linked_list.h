...
void makeNullList(node **root, node** cur, node** tail);
...
node *move_to_front(node **root, node **cur, node **tail,elementtype x);

node *transpose(node **root, node **cur, node **tail,elementtype x);
...
node *insertBeforeCur(node **root, node** cur, node** tail,elementtype e){
	if(*root == NULL || *cur == *root){ insertFirst(root,cur,tail,e); return *root; }
	node *new = makeNewNode(e);
	new->next = *cur;
	new->prev = (*cur)->prev;
	(*cur)->prev->next = new;
...
node *insert(node **root, node** cur, node** tail,elementtype e,int n){
	if(n == 0){	insertFirst(root,cur,tail,e);	return *root; }
	int i,num_node = 0;
	node *temp = NULL;
	...
	*cur = temp;
	insertBeforeCur(root,cur,tail,e);
	return *cur;
}
...
node *delete(node **root, node** cur, node** tail,int n)
{
  ...
  int i=0,j=1;
  node *temp = NULL;
  for(temp = (*root); temp->next != NULL; temp = temp->next) j++;
  if(n > j) { printf("Overfow\n"); return *root; }
  temp = *root;
  for(i = 0; i < n-1; i++) temp = temp->next;
  *cur = temp ;
  deleteCur(root,cur,tail);
  return *root;
}
...
node *move_to_front(node **root, node **cur, node **tail,elementtype x)
{
  ...
  for(temp = *root;temp != NULL && temp->e != x;temp = temp->next);
  if(temp->e == x)
    {
      if(temp == *root)
	{printf("Phan tu von da nam o vi tri dau tien\n");return *root;}
      insertFirst(root,cur,tail,x);
      *cur = temp;
      deleteCur(root,cur,tail);
    }
  return *root;
...
node *transpose(node **root, node **cur, node **tail,elementtype x)
{
  ...
  if(temp->e == x)
	...
	{printf("Phan tu von da nam o vi tri dau tien\n");return *root;}
      ...
      new = temp;
      *cur = temp->prev;
      insertBeforeCur(root,cur,tail,new->e);
      *cur = temp;
      deleteCur(root,cur,tail);
    }
  return *root;
