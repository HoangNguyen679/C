...
int main(int argc, char *argv[])
{
  ...
  BTA *btfile;
  btinit();
  btfile = btcrt("phonebook_db", 0, FALSE);
  ...
  for(i = 0; i < num_of_contact; i++)
    {
      ...
      char name[30];
      fscanf(f,"%s %s\n",name,contact.tel);
      btins(btfile, name, (char *)&contact,sizeof(contact_data));
    }
}
  ...
  while(1)
    {
      ...
      contact_data contact;
      int size_receive;
      if(btseln(btfile, name, (char *)&contact, sizeof(contact_data), &size_receive) == QNOKEY) break;
      printf("%-30s\t%-12s\n", name, contact.tel);
    }
...
void searchBtfile(BTA *btfile)
{
  ...
  scanf("%[^\n]",search_name);
  if (btsel(btfile, search_name, (char *)&contact, sizeof(contact_data), &rsize) == 0)
    {
      printf("\n\n%-30s\t%-12s\n\n","NAME","TEL");
...
void addBtfile(BTA *btfile)
{
  ...
  if (btsel(btfile, add_name, (char *)&add_contact, sizeof(contact_data), &rsize) == 0)
    {
