...
void InsertToBTS(nodeTree **T, elementType e){
  ...
  else if (e.grade < (*T)->element.grade) InsertToBTS(&(*T)->left, e);
...
nodeTree *SearchTree(nodeTree *T, elementType key){
  ...
  if (key.grade == T->element.grade) return T;
  if (key.grade < T->element.grade) return SearchTree(T->left, key);
...
void DeleteNode(nodeTree **T, elementType key){
  ...
  if (key.grade < (*T)->element.grade) {DeleteNode(&((*T)->left), key); return;}
  if (key.grade > (*T)->element.grade) {DeleteNode(&((*T)->right), key); return;}
...
int LowerThanKey(nodeTree *T, elementType key, int *numNodeLower){
  ...
  if (T->element.grade >= key.grade) return 0;
  (*numNodeLower)++;
  PrintDataTree(T->element);
