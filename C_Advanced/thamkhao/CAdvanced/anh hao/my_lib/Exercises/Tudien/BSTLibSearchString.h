...
void DeleteNode(nodeTree **T, elementType key){
  ...
  if (strcmp(key.eng,(*T)->element.eng) == -1) {DeleteNode(&((*T)->left), key); return;}
  if (strcmp(key.eng,(*T)->element.eng) == 1) {DeleteNode(&((*T)->right), key); return;}
...
void InsertToBTS(nodeTree **T, elementType e){
  ...
  if (strcmp(e.eng,(*T)->element.eng) == -1) {
  ...
  if (strcmp(e.eng,(*T)->element.eng) == 0) {
    // ((*T)->element.count)++;
    return;
  }
}
...
nodeTree *SearchTree(nodeTree *T, elementType key){
  ...
  if (strcmp(key.eng, T->element.eng) == 0) return T;
  if (strcmp(key.eng, T->element.eng) == -1) return SearchTree(T->left, key);
