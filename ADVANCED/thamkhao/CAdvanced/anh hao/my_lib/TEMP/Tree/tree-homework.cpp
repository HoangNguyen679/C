...
Node* root = NULL;
int height = 0, temp = 0, numberOfChild = 0;
...
int Height(Node* r){// compute the height of the node pointed by p
	// TODO
	// We use pre Order Traversal to compute height of a node
	if(r == NULL) return 0;
	temp++;	// plus temp when go to a child
	Node *p = r->leftmost_child;
	if(p == NULL){
		return 0;
	}
	...
	temp--;	// minus temp when go out to parent
	
	p = p->right_sibling;
	while(p != NULL){
		Height(p);
		temp--; // minus temp when go to next sibling because we already plus went go to right
		p = p->right_sibling;
	}

	if(temp > height) height = temp; // compare temp height with really height
	return height;
}
