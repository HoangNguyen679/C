...
void DeleteNode(nodeTree **T, elementType key){
	...
	if (key < (*T)->element) {DeleteNode(&((*T)->left), key); return;}
	if (key > (*T)->element) {DeleteNode(&((*T)->right), key); return;}
...
void deleteNode(nodeTree **pnode, elementType key){
	if (pnode == NULL)  return;
	if (*pnode == NULL) return;
	nodeTree **maxleft;
	if ((*pnode)->element == key){
		if ((*pnode)->right == NULL){
			if ((*pnode)->left == NULL){
				free(*pnode);
				*pnode = NULL;
		...
		(*pnode)->element = (*maxleft)->element;
		*maxleft = NULL;
		return;
	}
	if ((*pnode)->element < key){
		deleteNode(&(*pnode)->right,key);
		return;
	}
	deleteNode(&(*pnode)->left,key);
}

void InsertToBTS(nodeTree **T, elementType e){
	if (*T == NULL) *T = MakeNewNodeTree(e);
	else if (e < (*T)->element) InsertToBTS(&(*T)->left, e);
...
void InOrderTraverser(nodeTree *T){
 ...
 void PrintTree(nodeTree *T , int height){
	...
	nodeTree *p = T;
	for(i = 0; i < height+HeightTree(p); i++) printf("\t");
 	PrintDataTree(T->element);
	printf("\n");
...
void PrettyPrint(nodeTree *tree, char *prefix){
...
nodeTree *SearchTree(nodeTree *T, elementType key){
	...
	if (key == T->element) return T;
	if (key < T->element) return SearchTree(T->left, key);
