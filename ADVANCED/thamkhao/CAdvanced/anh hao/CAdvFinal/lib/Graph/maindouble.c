...
int main(int argc, char const *argv[])
{
	...
	AddEdge(g, 1, 2, 1.1, false);
	AddEdge(g, 1, 3, 2.2, false);
	AddEdge(g, 2, 4, 3.3456, false);
	//AddEdge(g, 1, 4, 1.1111, false);
	AddEdge(g, 3, 4, 4.5, false);


	// SHORTEST_PATH
	int path[100];
	int sizeofPath = 0;
	double totalLength = 0.000;
	ShortestPath(g, 1, 4, path, &sizeofPath, &totalLength);
	printf("Length = %g\n", totalLength);
	printf("Path: ");
	int i = 0;
