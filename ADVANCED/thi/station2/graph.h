...
Graph createGraph();
int addVertex(Graph g, int v, char *value, char *value2);
...
double shortest_path(Graph, int s, int t, int* path, double* length);
int DAG(Graph g);
...
int addVertex(Graph g, int v, char *value, char *value2)
{
	...
	jrb_insert_int(g.vertices2,v,new_jval_s(strdup(value2)));
	return 1;
      }
    return 0;
}
...
char *getVertex(Graph g, int v)
{
    if(g.edges == NULL || g.vertices == NULL)
      return NULL;
    JRB check = jrb_find_int(g.vertices2,v);
    if(check != NULL)
...
int deleteVertex(Graph g, int v)
{
  ...
  node = jrb_find_int(g.vertices2,v);
  jrb_delete_node(node);
